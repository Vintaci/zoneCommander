...
do
    ...
    function GCI:start()
        ...
        local function addGCIMenu(event, context)
			if event.id == world.event.S_EVENT_BIRTH and event.initiator and event.initiator.getPlayerName then
				...
				if player then
                    ...
                    if not context.groupMenus[groupid] then
                        ...
                        missionCommands.addCommandForGroup(groupid, '10 KM',  kmMenu, log(context.registerPlayer), context, player, unit, 10, true)
						missionCommands.addCommandForGroup(groupid, '20 KM',  kmMenu, log(context.registerPlayer), context, player, unit, 20, true)
                        missionCommands.addCommandForGroup(groupid, '25 KM',  kmMenu, log(context.registerPlayer), context, player, unit, 25, true)
						missionCommands.addCommandForGroup(groupid, '30 KM',  kmMenu, log(context.registerPlayer), context, player, unit, 30, true)
                        missionCommands.addCommandForGroup(groupid, '50 KM',  kmMenu, log(context.registerPlayer), context, player, unit, 50, true)

                        missionCommands.addCommandForGroup(groupid, '5 NM',  nmMenu, log(context.registerPlayer), context, player, unit, 5, false)
                        missionCommands.addCommandForGroup(groupid, '10 NM', nmMenu, log(context.registerPlayer), context, player, unit, 10, false)
						missionCommands.addCommandForGroup(groupid, '15 NM', nmMenu, log(context.registerPlayer), context, player, unit, 15, false)
						missionCommands.addCommandForGroup(groupid, '20 NM', nmMenu, log(context.registerPlayer), context, player, unit, 20, false)
                        missionCommands.addCommandForGroup(groupid, '25 NM', nmMenu, log(context.registerPlayer), context, player, unit, 25, false)

                        missionCommands.addCommandForGroup(groupid, 'Disable', menu, log(context.registerPlayer), context, player, unit, 0, false)
		...
		local gciMenu = {func = addGCIMenu, context = self}
		...
		function ev:onEvent(event)
			if event.id == world.event.S_EVENT_BIRTH and event.initiator and event.initiator.getPlayerName then
				...
				if player then
					...
					local err, msg = pcall(gciMenu.func, event, gciMenu.context)
					if not err then
		...
		local function getHeadingDiff(heading1, heading2) -- heading1 + result == heading2
                        ...
                        if wr > 0 then
                            for _,dt in pairs(dect) do
                                if dt:isExist() then
                                    ...
                                    if dist <= wr then
                                        local brg = math.floor(getBearing(data.unit:getPoint(), tgtPnt))

                                        local myPos = data.unit:getPosition()
                                        local tgtPos = dt:getPosition()
                                        local tgtHeading = math.deg(math.atan2(tgtPos.x.z, tgtPos.x.x))
                                        local tgtBearing = getBearing(tgtPos.p, myPos.p)
            
                                        local diff = math.abs(getHeadingDiff(tgtBearing, tgtHeading))
                                        local aspect = ''
                                        local priority = 1
                                        if diff <= 30 then
                                            aspect = "Hot"
                            ...
                            for _,tgt in ipairs(closeUnits) do
                                if data.metric then
                                    ...
                                    if km < 1 then
                                        ...
                                        msg = msg..math.floor(km+0.5)..'km at '
                                        msg = msg..(math.floor(tgt.altitude/250+0.5)*250)..'m, '
                                    ...
                                    if nm < 1 then
                                        ...
                                        msg = msg..'\n'..tgt.type..'  BRA: '..tgt.bearing..' for '
                                        msg = msg..math.floor(nm+0.5)..'nm at '
                                        msg = msg..(math.floor((tgt.altitude/0.3048)/1000+0.5)*1000)..'ft, '
                                        msg = msg..tostring(tgt.aspect)
                                    end
                                end
